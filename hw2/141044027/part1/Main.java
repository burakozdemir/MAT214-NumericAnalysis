    ...
    public void GESP(String file){
        ...
        for (int k = 0; k < equations.length-1; ++k){
            ...
            for (int i = k ; i < equations.length; ++i){
                ...
                if(r > maxValue){
                    maxValue = r;
                    maxIndis = i; // the largest indices
                }
            }
        ...
        for (int i = equations.length - 2; i >= 0; i--) {
            double sum = bmatrix[arr[i]];
            for (maxIndis = i; maxIndis < equations.length; maxIndis++) {
                sum = sum - equations[arr[i]][maxIndis] * resultMatrix[maxIndis];
            }
            resultMatrix[i] = sum/equations[arr[i]][i];
        }
        System.out.println();
        showMatrix();
        System.out.println();
